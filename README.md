...
## Process 
It started with a pretty frustrating observation. We were testing vision-language models on construction images — asking them things like "how far apart are these studs?" or "is this outlet at the right height?" — and the models would give these confident, articulate non-answers. They'd describe the scene beautifully and then either refuse to give a number or just make one up. It felt like talking to someone who had memorized every building code ever written but had never actually held a tape measure.
We knew VLMs understood construction. That wasn't the problem. The problem was that they had no way to connect what they were seeing in pixels to what anything actually measured in the real world. That gap felt like a solvable problem, and that's where the project started.
We talked to Daniele early on, and he pushed us to think about whether our work could be genuinely meaningful — not just a cool demo, but something with real research value and real utility. That conversation shifted how we framed the whole project. Instead of just building a pipeline and hoping it worked, we needed to ask a specific, testable question and design an experiment that would actually answer it. So we stepped back and asked: does injecting calibrated spatial measurements into a VLM prompt actually improve metric reasoning, and which parts of the pipeline are doing the work?
That question drove everything else. We designed a four-condition ablation before writing most of the code — image only, depth cues only, anchors only, and the full pipeline — so that whatever we found, we'd be able to say something specific about why. We used the ARKitScenes dataset for ground truth because it came with LiDAR-verified metric depth, which meant we didn't have to trust our own annotations.
Building the pipeline was actually the fun part. The core idea — use standardized construction objects like bricks and CMU blocks as calibration anchors, compute a pixel-to-meter scale, inject the measurements into the prompt — came together pretty naturally. YOLO for detection, Depth Anything V2 for plane separation, median aggregation for robustness, structured JSON into the VLM. Clean stages, clear interfaces.
What wasn't clean was debugging it on real images. Anchors on different wall planes were silently corrupting scale estimates. A single partially-occluded brick could throw off the whole calibration. Monocular depth was doing strange things when we tried to use it for anything metric. Every one of those failures pushed us toward more defensive design — depth-plane consistency checks, outlier rejection, multi-anchor aggregation. The system genuinely got better each time something broke in an interesting way.
The result that surprised us most was that adding depth cues without metric grounding made performance worse than just giving the model the raw image. It went from 1.27m MAE all the way up to 2.66m. That wasn't what we expected, but it made sense in hindsight — relative depth gives the model a false sense of certainty about scale, and it runs with it in the wrong direction. That finding ended up being the most important thing we could show: the bottleneck for VLMs in this domain isn't knowledge of construction standards. It's the absence of a real-world ruler.
